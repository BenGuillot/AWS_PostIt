...
async function createPostIt() {
  try{
    ...
    await knex.raw(`INSERT INTO postit (data,author, x, y, type, protect)
                          VALUES
                          ('Doubl click to post, post enough and you may find a shiny post it !', 'FAHEI','150','150','ShinyPostIt','prive')`);
  
        await knex.raw(`INSERT INTO postit (data,author, x, y, type, protect)
                          VALUES
                          ('Postit privé', 'LAU','10', '100', 'ShinyPostIt', 'public')`);
    
 //   await knex.select('*').from('postit').where('protect','public');
